...
optional_variables_names = ['node_features', 'collide_edge_index', 'collide_edge_features', 'align_edge_index',
                            'align_edge_features', 'target', 'predict', 'predict_order', 'target_shape', 'predict_probs']
...
def write_brick_layout_data(save_path, re_index,
                            ...
                            align_edge_features = None, target = None, prefix = None, predict = None, predict_order = None, target_shape = None,
                            predict_probs = None):
...
def load_brick_layout_data(save_path):
    ...
    return dic['re_index'], dic['node_features'], dic['collide_edge_index'], dic['collide_edge_features'], dic['align_edge_index'], \
                            dic['align_edge_features'], dic['target'], dic['predict'], dic['predict_order'], dic['target_shape'], dic['predict_probs']
...
def load_bricklayout(file_path, debugger, complete_graph):
    ...
    align_edge_features, y, predict, predict_order, target_polygon, predict_probs = load_brick_layout_data(file_path)
    ...
    output_layout = tiling.brick_layout.BrickLayout(debugger, complete_graph, node_features, collide_edge_index,
                               collide_edge_features, align_edge_index, align_edge_features, y, re_index)
...
def write_bricklayout(base_path, save_path, brick_layout, with_features = True):
    ...
    write_brick_layout_data(save_path = save_path,
                            ...
                            target = brick_layout.ground_truth,
...
def to_torch_tensor(device, node_feature, align_edge_index, align_edge_features, collide_edge_index, collide_edge_features, ground_truth):
    ...
    gt                    = None if (ground_truth is None or ground_truth.ndim == 0 or ground_truth.size == 0) else torch.from_numpy(ground_truth).float().to(device)
...
def recover_features_from_reindex(re_index, complete_graph):
    ...
    filtered_adj_edges = [edge for edge in complete_graph.adj_edges if
                               edge[0] in tiles_super_set and edge[1] in tiles_super_set]
    node_features, collide_edge_index, collide_edge_features, align_edge_index, align_edge_features, target, new_re_index = \
...
def generate_brick_layout_data(graph: TileGraph, super_tiles: list, collide_edges, adj_edges, target):
    # edge feature
    ...
    target = list(map(lambda i: re_index[i], target))
    target = [1 if i in target else 0 for i in range(len(super_tiles))]
    # print("Target")
    # print(target)

    return node_feature, np.array(edge_index_collide).T, np.array(super_edge_features_collide), \
           np.array(edge_index_align).T, np.array(super_edge_features_adj), np.array(target), re_index
